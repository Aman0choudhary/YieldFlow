# LEAN MAINNET DEPLOY for ~50 XLM budgets
# DO NOT RUN until user says: go mainnet deploy
# Correct entrypoints: init + set_blend_pool; optimized wasm only
...
function Invoke-Stellar {
  ...
  if ($code -ne 0) {
    Write-Host $text
    throw ("stellar failed exit " + $code)
  }
  return $text
}
...
function Get-ContractId([string]$Text) {
  ...
  if ($m.Count -eq 0) { throw "No contract id found" }
...
if (($BufferBps + $YieldBps) -ne 10000) { throw "BufferBps + YieldBps must equal 10000" }
...
if ($bal -lt 0) {
  ...
  if ($bal -lt $MinXlm) {
    throw ("Balance " + $bal + " XLM is below MinXlm " + $MinXlm)
  }
  if ($bal -lt 50) {
...
if (-not $SkipBuild) {
  Write-Host "[1/6] Building optimized wasm..." -ForegroundColor Cyan
  Push-Location contracts
  try {
    Invoke-Stellar @("contract","build","--optimize") | ForEach-Object { Write-Host $_ }
  } finally { Pop-Location }
} else {
  Write-Host "[1/6] SkipBuild - using existing optimized wasm" -ForegroundColor Yellow
}
...
$common = @("--network", $Network, "--rpc-url", $RpcUrl, "--network-passphrase", $NetworkPassphrase, "--source-account", $SourceAccount)

Write-Host "[2/6] Deploy streaming..." -ForegroundColor Cyan
...
Write-Host "[3/6] Deploy vault..." -ForegroundColor Cyan
...
Write-Host "[4/6] Init streaming..." -ForegroundColor Cyan
Invoke-Stellar (@("contract","invoke","--id",$streamingId) + $common + @("--","init","--employer",$employer,"--withdrawal_controller",$vaultId)) | ForEach-Object { Write-Host $_ }

Write-Host "[5/6] Init vault..." -ForegroundColor Cyan
Invoke-Stellar (@("contract","invoke","--id",$vaultId) + $common + @("--","init","--employer",$employer,"--withdrawal_controller",$employer,"--streaming_contract",$streamingId,"--token",$TokenContractId,"--buffer_bps","$BufferBps","--yield_bps","$YieldBps")) | ForEach-Object { Write-Host $_ }

Write-Host "[6/6] set_blend_pool..." -ForegroundColor Cyan
$blendArg = [char]34 + $BlendPoolId + [char]34
Invoke-Stellar (@("contract","invoke","--id",$vaultId) + $common + @("--","set_blend_pool","--blend_pool",$blendArg)) | ForEach-Object { Write-Host $_ }
